...
from src.impute.dataset_for_mit import DatasetForMIT
...
class _SAITS(nn.Module):
    ...
    def impute(self, inputs):
        ...
        X_tilde_1 = F.relu(self.reduce_dim_z(enc_output))
        ...
        X_tilde_2 = F.relu(self.reduce_dim_gamma(F.relu(self.reduce_dim_beta(enc_output))))
        ...
        # DD modification
        X_tilde_3 = F.relu((1 - combining_weights) * X_tilde_2 + combining_weights * X_tilde_1)
        X_c = (
...
class SAITS(BaseNNImputer):
    ...
    def fit(self, X):
        training_set = DatasetForMIT(X, idx_list=self.train_idx_list, window_size=self.n_steps, stride=self.stride)
        training_loader = DataLoader(
            training_set, batch_size=self.batch_size, shuffle=True
        )

        if self.val_idx_list is None:
